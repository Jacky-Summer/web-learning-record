...
class Compilation {
  ...
  build(callback) {
    ...
    let context = toUnixPath(process.cwd())
    for (const entryName in entry) {
      // 找到入口文件的绝对路径
      let entryFilePath = path.posix.join(context, entry[entryName]) // path.posix：允许在任意操作系统上使用linux的方式来操作路径
  ...
  buildModule(name, modulePath) {
    ...
    const { code } = generator(ast) //根据改造后的语法树生成源代码
